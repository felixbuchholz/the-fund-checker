  ...
  <div>
    ...
    >
      ...
      <v-card-text>
        <!--  -->
        <v-btn-toggle
          class="width-100pc transparent"
          v-model="showManagerModel"
          multiple
          dense
        >
          <div class="width-100pc">
            ...
            >
              ...
              <v-btn @click="onClick" class="white" rounded outlined>
                <v-icon>{{
                  showManager.includes(indexManager) ? "mdi-eye-off" : "mdi-eye"
                }}</v-icon
                >&nbsp;
                <span class="caption text-capitalize">{{
                  showManager.includes(indexManager) ? "Hide" : "Show"
                }}</span>
              </v-btn>
            </div>
          </div>
        ...
        <div class="my-4 d-flex justify-space-between align-center">
          ...
          <v-btn-toggle v-model="showESGModel" multiple rounded dense>
            <v-btn @click="onClick" class="white">
              <v-icon>{{
                showESG.includes(0) ? "mdi-eye-off" : "mdi-eye"
              }}</v-icon
              >&nbsp;
            ...
            <v-btn class="white">
              <v-icon>{{
                showESG.includes(1) ? "mdi-eye-off" : "mdi-eye"
              }}</v-icon
              >&nbsp;
        ...
        <div class="d-flex justify-space-between align-center">
          ...
          >
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn @click="onClick" color="white" dark v-on="on">
                  <v-icon>{{
                    toggleAllModel.includes(0) ? "mdi-eye-off" : "mdi-eye"
                  }}</v-icon>
                </v-btn>
              </template>
              <span>{{
                toggleAllModel.includes(0)
                  ? "Hide all options"
                  : "Show all options"
              }}</span>
            </v-tooltip>
            ...
            <v-tooltip top>
              ...
              <span>{{
                toggleAllModel.includes(1) ? "Unfold funds" : "Overlap funds"
              }}</span>
            </v-tooltip>
